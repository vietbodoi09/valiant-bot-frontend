...
import { 
  ...
  Lock, Unlock, Download, Upload, Activity
} from 'lucide-react';
...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  const fetchKeys = useCallback(async (key: string) => {
    try {
      const res = await fetch(`${API_URL}/api/admin/keys?admin_key=${encodeURIComponent(key)}`);
      if (res.ok) {
  ...
  const handleLogin = async () => {
    ...
    setLoginError('');
    try {
      const res = await fetch(`${API_URL}/api/admin/verify?admin_key=${encodeURIComponent(adminKey)}`);
      if (res.ok) {
    ...
    try {
      const res = await fetch(`${API_URL}/api/admin/keys`, {
        ...
        body: JSON.stringify({
          admin_key: adminKey,
          name: newKeyName,
          max_devices: parseInt(newKeyDevices) || 3
        })
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const revokeKey = async (id: string) => {
    ...
    await fetch(`${API_URL}/api/admin/keys/${id}/revoke`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const reactivateKey = async (id: string) => {
    await fetch(`${API_URL}/api/admin/keys/${id}/reactivate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ admin_key: adminKey })
    });
  ...
  const deleteKey = async (id: string) => {
    ...
    await fetch(`${API_URL}/api/admin/keys/${id}?admin_key=${encodeURIComponent(adminKey)}`, { method: 'DELETE' });
    await fetchKeys(adminKey);
  };

  const exportKeys = async () => {
    try {
      const res = await fetch(`${API_URL}/api/admin/keys/export?admin_key=${encodeURIComponent(adminKey)}`);
      if (res.ok) {
        const data = await res.json();
        ...
        a.href = url;
        a.download = `valbot-keys-backup-${new Date().toISOString().slice(0, 10)}.json`;
        a.click();
  ...
  const importKeys = async () => {
    try {
      ...
      if (!Array.isArray(keysData)) { setImportResult('Invalid format: expected {keys: [...]}'); return; }
      
      const res = await fetch(`${API_URL}/api/admin/keys/import`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ admin_key: adminKey, keys: keysData })
      });
      if (res.ok) {
        const data = await res.json();
        setImportResult(`Imported ${data.imported} keys, skipped ${data.skipped} duplicates`);
        await fetchKeys(adminKey);
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-white/30 text-sm mt-1">
              {keys.length} keys total · {activeCount} active
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant="outline" onClick={() => fetchKeys(adminKey)}
          ...
          <Card className="mb-6 bg-white/[0.03] border-white/10">
            <CardContent className="p-5 space-y-3">
              ...
              <textarea value={importJson} onChange={e => setImportJson(e.target.value)}
                placeholder='Paste JSON backup here or upload file...'
            ...
            {!showCreate && !generatedKey ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-emerald-400 text-sm">
                  <CheckCircle className="w-4 h-4" /> Key created successfully!
                </div>
                <div className="flex items-center gap-2">
                  <code className="flex-1 bg-black/50 px-4 py-3 rounded-lg text-emerald-400 font-mono text-sm border border-emerald-500/20">
                    {generatedKey}
                  </code>
                  <Button size="sm" onClick={() => copyToClipboard(generatedKey)}
                    className={copiedKey ? 'bg-emerald-600' : 'bg-white/10 hover:bg-white/20'}>
                    {copiedKey ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                  </Button>
                </div>
                <p className="text-white/30 text-xs">Save this key now — it won't be shown again in full.</p>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-white/50 text-xs">Key Name</Label>
                    <Input value={newKeyName} onChange={e => setNewKeyName(e.target.value)}
                      placeholder="e.g. Phone, Laptop"
                      className="bg-black/50 border-white/10 text-white text-sm mt-1" />
                  </div>
                  <div>
                    <Label className="text-white/50 text-xs">Max Devices</Label>
                ...
                <div className="flex gap-2">
                  <Button size="sm" onClick={createKey} disabled={!newKeyName.trim()}
                    className="bg-emerald-500 hover:bg-emerald-600 text-white">Create</Button>
                  <Button size="sm" variant="outline" onClick={() => setShowCreate(false)}
                    className="border-white/10 text-white/60">Cancel</Button>
                </div>
              </div>
            )}
          </CardContent>
            ...
            {(['all', 'active', 'revoked'] as const).map(f => (
              <button key={f} onClick={() => setFilterStatus(f)}
                className={cn('px-3 py-1.5 text-xs rounded-lg transition-colors capitalize',
                  filterStatus === f ? 'bg-emerald-500/20 text-emerald-400' : 'text-white/30 hover:text-white/60')}>
                {f}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-2">
          {filteredKeys.length === 0 ? (
            <div className="text-center py-12 text-white/20 text-sm">
              {keys.length === 0 ? 'No keys created yet' : 'No matching keys'}
            </div>
          ) : filteredKeys.map(k => (
            <div key={k.id} className={cn(
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1 text-[11px] text-white/25">
                  ...
                  {k.last_used && <>
                    <span>·</span>
                    <span>Last: {new Date(k.last_used).toLocaleDateString()}</span>
                  </>}
                </div>
              </div>
              ...
              <div className="flex items-center gap-1">
                {k.is_active ? (
                  <Button size="sm" variant="ghost" onClick={() => revokeKey(k.id)}
                    className="text-white/20 hover:text-yellow-400 h-8 w-8 p-0">
                    <Ban className="w-3.5 h-3.5" />
                  </Button>
                ) : (
                  <Button size="sm" variant="ghost" onClick={() => reactivateKey(k.id)}
                    className="text-white/20 hover:text-emerald-400 h-8 w-8 p-0">
